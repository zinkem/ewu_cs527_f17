...
function init() {
  ...
  var stuff = searchParams.get('stuff');

  var content_div = new_div('content');
  place_content('/api/hashstuffs/stats/'+stuff, content_div,
                function(){
                  place_content('/api/hashstuffs/tweets/'+stuff, content_div);
                });

  var top_div = new_div('header');
  top_div.innerHTML = 'Top 5 Most '+stuff;
