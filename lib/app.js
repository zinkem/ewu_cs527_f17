...
var fetch_timeseries = function(event, hashtag) {
  ...
  console.log(utime, oldest);
  connection.query("select t.create_time from hashtags h inner join tweets t on h.tweet_id = t.id and h.label= ? and t.create_time > ? ", [ hashtag, oldest] , sql_response_handler(event));
};
