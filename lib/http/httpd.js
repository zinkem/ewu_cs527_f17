...
var Httpd = function(opts) {
  ...
  this.server = http.createServer(function(req, res) {
    ...
    var http_client_ip = req.connection.remoteAddress;
    ...
    if( api === "api" ){
      ...
      var registered = self.eventNames();
      if( self.eventNames().includes(event) ) {
        self.emit(event, apiOpts)
      } else {
        res.end('404 unregistered api');
      }
    } else {
      if( api === "console" &&
